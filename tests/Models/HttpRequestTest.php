...
final class HttpRequestTest extends PHPUnit\Framework\TestCase {
    public function test_post_from_cgi(): void {
        $env = array(
            ...
            'REQUEST_SCHEME' => 'http',
            'SCRIPT_URL' => '/path/to/hello/world?one=1&two=2',
        ...
        static::assertSame('HTTP/1.1', $request->protocol);
        static::assertSame(
            'POST /path/to/hello/world?one=1&two=2 HTTP/1.1',
        ...
        foreach ($header_lines as $value) {
            static::assertContains($value, $request->header_lines());
        }

        static::assertSame('hello world!', $request->body);
    }
